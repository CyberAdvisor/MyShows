...
- **The hosted copy at myshows.fyi also loads Statcounter**, a third-party web analytics service, installed to help debug access issues (e.g. confirming the site is actually reachable, spotting outages or broken deploys) rather than to profile individual visitors. It reports page-view/visit data back to Statcounter; the visible tracking badge is set to invisible (`sc_invisible=1`) but the underlying tracking script still runs. It never sees your watch list. If you self-host this `index.html`, you can remove the Statcounter script block from the `<head>` if you don't want it included.
