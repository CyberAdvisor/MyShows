...
## Statcounter analytics beacon

`index.html`'s `<head>` includes Statcounter tracking (two `<script>` tags plus a `<noscript>` fallback), reporting page-view/visit data for the hosted copy at myshows.fyi. `sc_invisible=1` hides the visible tracking badge but the tracking script itself still runs. It never touches `state.library` or anything in `localStorage`. Documented in the README's data-storage section.

**Placement matters for testing, not just style.** It's deliberately placed in `<head>`, *before* the main inline `<script>` block, not near `</body>` where analytics snippets usually go. `app.test.js` extracts the app's real logic with `html.match(/<script>([\s\S]*)<\/script>/)[1]` - a greedy match from the first literal `<script>` (no attributes) to the *last* `</script>` in the whole file. If any of the Statcounter `<script>` tags (or any other `<script>` tag) were placed after the main script instead, its closing `</script>` would become the new "last" one, and the regex would sweep that tag's opening `<script ...>` text into the extracted body, breaking `window.eval(scriptBody)` with a syntax error and failing every test. If you add another `<script>` tag to this file for any reason, put it above the main inline script, not below it.
